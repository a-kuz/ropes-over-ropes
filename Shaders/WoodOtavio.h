#ifndef WOOD_OTAVIO_H
#define WOOD_OTAVIO_H

#include "WoodCommon.h"

static float noiseGenWood(float3 p) {
    float wave0 = 0.0;
    float wave1 = 0.0;
...
static float reprampWood(float x) {
    return pow(sin(x) * 0.5 + 0.5, 8.0) + cos(x) * 0.7 + 0.7;
}

static float3 otavioWoodTexture(float2 worldXY, float seed) {
    ...
    float2 offset = float2(wc_hash21(float2(seed, seed * 5.37)) * 40.0 - 20.0,
                           wc_hash21(float2(seed * 2.83, seed)) * 40.0 - 20.0);
    float3 pos = float3((rotated + offset) * 1.1, floor(fract(seed * 0.41) * 8.0));

    float scaleVar = mix(0.8, 1.2, wc_hash21(float2(seed * 1.47, 0.0)));
    pos.xy *= scaleVar;

    float rings = reprampWood(length(pos.xz + float2(noiseGenWood(pos * float3(8.0, 1.5, 8.0)),
                                                       noiseGenWood(-pos * float3(8.0, 1.5, 8.0) + 4.5678)) * 0.05) * 64.0) / 1.8;
    rings -= noiseGenWood(pos * 1.0) * 0.75;

    float3 colDark, colMid, colLight;
    wc_woodPalette(seed, colDark, colMid, colLight);

    float3 texColor = mix(colDark * 0.95, colLight * 0.4, saturate(rings)) * 1.5;
    texColor = max(float3(0.0), texColor);
    float rough = noiseGenWood(pos * 64.0 * float3(1.0, 0.2, 1.0)) * 0.1 + 0.9;
    texColor *= rough;
    return saturate(texColor);
}

static float3 otavioBoardWoodTexture(float2 worldXY, float elevation, float seed) {
    float seedAngle = seed * 2.718;
    float sa = sin(seedAngle), ca = cos(seedAngle);
    float2 rotated = float2(worldXY.x * ca - worldXY.y * sa,
                            worldXY.x * sa + worldXY.y * ca);
    ...
    float3 pos = float3((rotated + offset) * 0.5, elevation * 2.0 + seed * 7.0);
