...
enum HookGeometryCalculator {
    ...
    static func calculateHookSequenceGeometry(
        ...
        // Caller is responsible for passing already-resolved B1/B2 endpoints
    ...
    private static func calculateHookSequenceGeometryInternal(
        ...
        // B endpoints are already resolved by caller — no swap needed
        let S1 = (A1 + B1) * 0.5
        let S2 = (A2 + B2) * 0.5
        ...
        if sigmaLen < 0.05 {
            ...
            if let intersection = lineIntersection(A1, A2, B1, B2) {
                mid = intersection
                if shouldLog {
                ...
                if shouldLog { logger.info("    lines are PARALLEL, using A1-B1 direction") }
        ...
        let minStep = R * 2.1  // Guarantee crossings by ensuring centers don't overlap
        let step = max(minStep, min(baseStep * repulse * stepMultiplier, stepLimit))
