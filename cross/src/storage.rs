...
#[cfg(target_arch = "wasm32")]
pub fn save_settings_to_storage(settings_json: &str) {
    let storage: Option<web_sys::Storage> =
        web_sys::window().and_then(|w| w.local_storage().ok().flatten());
    if let Some(s) = storage {
        let _ = s.set_item("uzls_settings", settings_json);
    }
}
...
pub fn load_level_from_storage() -> Option<usize> {
    let storage: Option<web_sys::Storage> =
        web_sys::window().and_then(|w| w.local_storage().ok().flatten());
    let val: Option<String> = storage.and_then(|s| s.get_item("uzls_level").ok().flatten());
    val.and_then(|v| v.parse::<usize>().ok())
        .filter(|&v| v >= 1)
}

#[cfg(target_arch = "wasm32")]
pub fn load_settings_from_storage() -> Option<String> {
    let storage: Option<web_sys::Storage> =
        web_sys::window().and_then(|w| w.local_storage().ok().flatten());
    storage.and_then(|s| s.get_item("uzls_settings").ok().flatten())
}
...
fn settings_file_path() -> Option<std::path::PathBuf> {
    dirs::data_local_dir().map(|d| d.join("uzls4").join("settings.json"))
}
...
pub fn save_settings_to_storage(settings_json: &str) {
    if let Some(path) = settings_file_path() {
        ...
        let _ = std::fs::write(&path, settings_json);
    }
}
...
#[cfg(not(target_arch = "wasm32"))]
pub fn load_settings_from_storage() -> Option<String> {
    settings_file_path().and_then(|p| std::fs::read_to_string(p).ok())
}
