#version 300 es
// Shadow pass vertex shader for ropes, holes, and boards.
// Use uShadowMode uniform: 0 = rope, 1 = hole (instanced), 2 = board
...
uniform int uShadowMode; // 0 = rope, 1 = hole (instanced), 2 = board

// Rope / board attributes (position is always at 0)
layout(location = 0) in vec3 aPosition;
layout(location = 1) in vec3 aNormal;
layout(location = 2) in vec3 aColor;     // rope only, unused in shadow
layout(location = 3) in vec2 aTexCoord;  // rope only
layout(location = 4) in vec4 aParams;    // rope only

// Hole per-instance attribute
layout(location = 5) in vec4 aInstanceData; // (x, y, elevation, radius)

void main() {
    if (uShadowMode == 0) {
        // --- Rope shadow (same displacement as ropeShadowVertex in Metal) ---
        ...
        gl_Position = uLightViewProj * vec4(displaced, 1.0);

    } else if (uShadowMode == 1) {
        // --- Hole shadow (instanced) ---
        ...
        gl_Position = uLightViewProj * vec4(wp, 1.0);
